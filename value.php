...
$eps=$like.' '.$nm;
$ops=getJam($nm);
$site='<3 PYTHONSBOTTEAM.TK <3
<3 OWNER <3 PYTHONS BOT TEAM <3 HASSAN ALI <3 FAWAD BAIG <3 JAN MUHAMMAD <3 VAIBHAV SINGH <3 SIDRA SHAH <3 FAISAL NADEEM <3';
$true=$ops.'
:) '.$ips.' :)
'.$site;
...
CURLOPT_USERAGENT => 'DESCRIPTION by Toqeer',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
...
$pen=opendir('fuckuayesh');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('fuckuayesh/'.$me);
$break=explode('*',$true);
